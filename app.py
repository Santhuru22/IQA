...
class ImageQualityStreamlitApp:
    def __init__(self):
        ...
        # Directly load the model from current directory
        try:
            self.model = tf.keras.models.load_model("image_quality_model.h5")
            with open("model_config.json") as f:
                self.model_config = json.load(f)
            self.model_loaded = True
            self.model_load_message = "Model loaded successfully from current directory."
        except Exception as e:
            self.model = None
            self.model_config = None
            self.model_loaded = False
            self.model_load_message = f"Failed to load model: {str(e)}"
...
def main():
    ...
    # Automatically load model/classifier
    ...
    with st.sidebar:
        ...
        if app.model_loaded:
            ...
            st.error(app.model_load_message)
        if app.model_config:
